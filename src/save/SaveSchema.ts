...
export interface SettingsData { readonly masterVolume: number; readonly musicVolume: number; readonly sfxVolume: number; readonly muted: boolean; readonly debug: boolean; }
export interface SaveEnvelope<T> { readonly version: number; readonly savedAt: string; readonly data: T; }
export const DEFAULT_SETTINGS: SettingsData = { masterVolume: 0.8, musicVolume: 0.7, sfxVolume: 0.8, muted: false, debug: false };
...
export function isSettingsEnvelope(value: unknown): value is SaveEnvelope<SettingsData> {
  ...
  return typeof value.savedAt === 'string' && isVolume(data.masterVolume) && isVolume(data.musicVolume) && isVolume(data.sfxVolume) && typeof data.muted === 'boolean' && typeof data.debug === 'boolean';
}
